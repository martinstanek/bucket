...
public sealed class BucketWorkerEndToEndTests
{
    ...
    public async Task Execute_Install_InstallationExecuted()
    {
        ...
        context.DockerService.Verify(v => v.IsDockerRunningAsync(), Times.Once);
    }
    ...
    public async Task Execute_Bundle_BundlingExecuted()
    {
        ...
        context.HostLifeTime.Verify(v => v.StopApplication(), Times.Once);
        context.DockerService.Verify(v => v.IsDockerRunningAsync(), Times.Once);
        context.DockerService.Verify(v => v.PullImageAsync(It.IsAny<string>()), Times.AtLeastOnce);
        context.DockerService.Verify(v => v.SaveImageAsync(It.IsAny<string>(), It.IsAny<string>()), Times.AtLeastOnce);
    }
